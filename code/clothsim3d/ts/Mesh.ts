...
export default class Mesh {
    ...
    static pointAwayFrom(triangle:Triangle, point:Vec3=new Vec3()) {
    ...
    render(renderer:Renderer, context:CanvasRenderingContext2D) {
    ...
    static BuildIcosahedron():Mesh {
      ...
      function triangle_from_indices(i, j, k) {
        return new Triangle(mesh.points[i],
                            mesh.points[j],
                            mesh.points[k],
                            RED);
      }
    ...
    RefineSphericalMesh(iteration:number):void {
      let new_triangles = [];
    ...
    static BuildSphere(iterations:number=2, type:number=2):Mesh {
